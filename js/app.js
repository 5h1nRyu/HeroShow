 ...
 function render(ms) {
  ...
  if (introActive) {
   // 人物以原始比例、满屏高度直线进入；居中后大幅逼近镜头。
   const portraitIn = clamp(position / 280);
   const portraitScale = clamp((position - 280) / 220);
   $('intro-portrait-wrap').style.transform =
           `translate3d(${-1000 * (1 - portraitIn)}px,0,0) scale(${1 + 1.35 * portraitScale})`;
